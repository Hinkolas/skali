...
func seedBackupScope(t *testing.T) *fakeInstall {
	...
	install.backups["p1"] = []client.BackupSnapshot{
		...
		{ID: testSnapshotStaging, Environment: "staging", CreatedAt: "2026-08-14T09:30:00Z",
			RevisionChecksum: "sha256:0123456789abcdef", Trigger: "scheduled", Databases: 1, Bytes: 512},
...
func TestBackupLsListsWholeProjectWithHeader(t *testing.T) {
	...
	// Where each snapshot came from: a person, or the environment's schedule.
	require.Regexp(t, `(?m)^`+testSnapshotProduction+`  production +manual +2026`, out)
	require.Regexp(t, `(?m)^`+testSnapshotStaging+`  staging +scheduled +2026`, out)
}
...
func TestBackupRemoveConfirmsAndDeletes(t *testing.T) {
	...
	require.Regexp(t, `(?m)^snapshot +`+testSnapshotStaging+` \(staging, .*scheduled, 512B\)$`, out.String())
