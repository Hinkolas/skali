...
// joinTokenPayload is the composite token body.
...
// parts, a raw k3s token passes through with no pull credential so a
// manually minted token still joins (with a visible warning about
// unauthenticated registry pulls).
func decodeJoinToken(token string) (k3sToken, pullSecret string, err error) {
	...
	return payload.K3s, payload.Pull, nil
}
