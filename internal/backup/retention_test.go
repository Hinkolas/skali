...
func snapshotManifest(id, environment, trigger, policy string, created time.Time) *Manifest {
	return &Manifest{
...
func TestRetentionCandidates(t *testing.T) {
	...
	otherPolicy := snapshotManifest("weekly-old", "production", TriggerScheduled, "weekly", now.Add(-30*day))
	newest := snapshotManifest("daily-newest", "production", TriggerScheduled, "daily", now.Add(-10*day))
	protectedOld := snapshotManifest("daily-restoring", "production", TriggerScheduled, "daily", now.Add(-25*day))
	oldA := snapshotManifest("daily-old-a", "production", TriggerScheduled, "daily", now.Add(-26*day))
	oldB := snapshotManifest("daily-old-b", "production", TriggerScheduled, "daily", now.Add(-40*day))
	...
	candidates := retentionCandidates(
		[]*Manifest{oldManual, otherPolicy, oldB, newest, protectedOld, oldA, legacy},
		"daily", now.Add(-7*day), map[string]bool{"daily-restoring": true})
	...
	require.ElementsMatch(t, []string{"daily-old-a", "daily-old-b"}, ids,
		"manual, other-policy, newest, protected and legacy snapshots are never candidates")

	// Even a window shorter than the gap to the newest keeps the newest.
	single := retentionCandidates([]*Manifest{newest}, "daily", now, nil)
	require.Empty(t, single)

	// A snapshot inside the window is kept even when it is not the newest.
	fresh := snapshotManifest("daily-fresh", "production", TriggerScheduled, "daily", now.Add(-3*day))
	fresher := snapshotManifest("daily-fresher", "production", TriggerScheduled, "daily", now.Add(-day))
	inside := retentionCandidates([]*Manifest{fresh, fresher, oldA}, "daily", now.Add(-7*day), nil)
...
// snapshots of the policy go, everything else stays, and the step log
// explains the sweep.
func TestApplyRetentionSweepsOnlyExpiredPolicySnapshots(t *testing.T) {
	...
	manifests := []*Manifest{
		...
		snapshotManifest("daily-old-a", "production", TriggerScheduled, "daily", now.Add(-9*day)),
		snapshotManifest("daily-old-b", "production", TriggerScheduled, "daily", now.Add(-8*day)),
		snapshotManifest("daily-kept", "production", TriggerScheduled, "daily", now.Add(-2*day)),
		snapshotManifest("daily-newest", "production", TriggerScheduled, "daily", now),
		snapshotManifest("staging-old", "staging", TriggerScheduled, "daily", now.Add(-30*day)),
	}
	...
	run, err := f.journal.CreateRun(ctx, journal.RunInput{
		Kind: KindBackup, ProjectID: f.projectID, EnvironmentID: f.environmentID, Actor: ScheduleActor("daily"),
	})
	require.NoError(t, err)
	...
	row := &store.Backup{ProjectName: "demo", EnvironmentName: "production", Trigger: TriggerScheduled, Policy: "daily"}
	...
	require.NoError(t, scope.step(ctx, "retention", "Apply retention", func(ctx context.Context, log *stepLog) error {
		return f.controller.applyRetention(ctx, log, bctx, "daily", 7*day, now)
	}))

	for _, id := range []string{"manual-old", "daily-kept", "daily-newest"} {
