...
func TestCanonicalDomainNamesTheOffendingCharacter(t *testing.T) {
	...
	cases := []struct{ value, want string }{
		...
		{"b1.eplan.localhost:3000", "must be a DNS hostname without a port: character 19 starts :3000; routes reach the application through the route's port field"},
		{"localhost:80", "without a port: character 10 starts :80"},
		{"a:b.localhost", "character 2 is ':'"},
