...
// plus one run feed per readable environment for the environments card.
// State and health come with the environments themselves (the layout asks
// for the listing's summary), so no row costs a status projection. A
// locked environment gets no requests: its contents are refused anyway.
	...
	const insightEntries = await Promise.all(
		environments
			...
			.map(async (e): Promise<[string, EnvironmentInsight]> => {
				const runsRes = await apiFetch(fetch, `/v1/environments/${e.id}/runs`);
