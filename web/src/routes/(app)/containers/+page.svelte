<script lang="ts">
	...
	import type { NodeImage } from '$lib/types/nodes';
	...
	// Read-only node truth: what the engines report, verbatim. Changes go
	// through workloads (and the registry) — this page is the drift-debugging
	// window, deliberately without write actions.
	...
	const ctrGrid = 'grid-cols-[1.1fr_2.4fr_0.9fr_1.2fr_1.5fr]';
...
<PageHeader title="Containers">
	{#snippet subtitle()}
		What the engines report — read-only; workloads drive changes.
		...
		{#if containers.length === 0}
			<EmptyState
				...
				description={filtered
					...
					: 'Containers appear here as heartbeats report them; create some by declaring a workload.'}
			/>
		{:else}
