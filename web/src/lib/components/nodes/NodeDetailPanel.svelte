<script lang="ts">
	...
	/** "3 containers · 2 images · 1 volume" from the list payload's counts. */
	const engineSummary = $derived.by(() => {
		const e = node.engine;
		if (!e) return null;
		const part = (n: number, word: string) => `${n} ${word}${n === 1 ? '' : 's'}`;
		return `${part(e.containers, 'container')} · ${part(e.images, 'image')} · ${part(e.volumes, 'volume')}`;
	});
</script>
...
<div class="flex-1 overflow-y-auto px-4.5 pb-4">
	{#if tab === 'overview'}
		...
		<!-- engine at a glance; the Containers page is the full surface -->
		<h3 class="text-text-ghost mt-5 mb-1.5 text-[10px] font-semibold tracking-[0.12em] uppercase">
			Engine
		</h3>
		<a
			href="/containers?node={node.id}"
			class="group flex items-center justify-between rounded-[10px] bg-white/3 px-3 py-2.5 transition-colors hover:bg-white/5"
		>
			<span class="font-mono text-text-primary text-[12px]">{engineSummary ?? '—'}</span>
			<span class="text-accent-nav flex flex-none items-center gap-1 text-[11.5px]">
				View
